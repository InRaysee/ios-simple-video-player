...
struct LocalMediaPage: View {
    ...
    @State private var newLocalMedia: LocalMedia?
        ...
        VStack(spacing: 0) {
            ...
            ZStack {
                if let player {
                    StreamPlayer(player: player)
                        ...
                        .overlay(alignment: .bottom) {
                            StreamSeekerView(size)
                        }
                        .overlay {
                            Text("Stream source unselected.")
                                .fontWeight(.bold)
                                .foregroundStyle(.white)
            ...
            Form {
                Section("Select a media source to load:") {
                    ...
                    .sheet(item: $newLocalMedia) { localMedia in
                        NavigationStack {
                            LocalMediaDetail(localMedia: localMedia)
    ...
    private func deleteMedia(offsets: IndexSet) {
        withAnimation {
            for index in offsets {
                context.delete(localMedias[index])
            }
        }
    }
    ...
    // Stream Seeker View
    @ViewBuilder
    func StreamSeekerView(_ streamSize: CGSize) -> some View {
                ...
                .gesture(
                    DragGesture()
                        ...
                        .onChanged({ value in
                            ...
                            let calculatedProgress = (translationX / streamSize.width) + lastDraggedProgress
        ...
        ZStack {
            HStack(spacing: 25) {
                ...
                Button {
                    if isPlaying {
                        player?.pause()
                        // Cancelling timeout task when the stream is paused
                        if let timeoutTask {
                            timeoutTask.cancel()
                        }
                    } else {
